...
def main(config: _config.TrainConfig):
    ...
    data_loader = _data_loader.create_data_loader(
        ...
        # pi05/create_data_loader now reads config.num_workers internally, so
        # passing num_workers here raises: unexpected keyword argument 'num_workers'.
    ...
    for step in pbar:
        ...
        if should_console_log:
            stacked_console_infos = common_utils.stack_forest(console_infos)
            reduced_console_info = jax.device_get(jax.tree.map(jnp.mean, stacked_console_infos))
            info_str = ", ".join(f"{k}={v:.4f}" for k, v in reduced_console_info.items())
        ...
        if step % config.log_interval == 0:
            ...
            reduced_wandb_info = jax.device_get(jax.tree.map(jnp.mean, stacked_wandb_infos))
            wandb.log(reduced_wandb_info, step=step)
            wandb_infos = []
        batch = next(data_iter)
